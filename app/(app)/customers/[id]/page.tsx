...
export default async function CustomerDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="mb-5">
        <Link href="/customers" className="text-xs text-gray-500 hover:text-orange-500 transition-colors font-medium uppercase tracking-wider">
          ← Customers
        </Link>
      </div>

      {/* Header card */}
      <div className="bg-[#1a1a1a] border border-white/10 rounded-[20px] p-6 mb-5">
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              ...
              {totalLbs.toLocaleString()} lbs total &middot; {rewardsEarned} gift card{rewardsEarned !== 1 ? 's' : ''} earned
            </p>
          </div>
          {pendingRewards.length > 0 && (
            <div className="bg-orange-500/15 text-orange-500 border border-orange-500/20 px-4 py-2 rounded-full text-sm font-bold shrink-0 uppercase tracking-wide">
      ...
      {customer.rewards.length > 0 && (
        <div className="bg-[#1a1a1a] border border-white/10 rounded-[20px] overflow-hidden mb-5">
          <div className="px-5 py-4 border-b border-white/10">
            <h2 className="font-semibold text-white text-sm uppercase tracking-wider">Reward History</h2>
          </div>
            ...
            <tbody className="divide-y divide-white/5">
              {customer.rewards.map((r) => (
                <tr key={r.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-5 py-3 text-gray-300">{new Date(r.earnedAt).toLocaleDateString()}</td>
                  <td className="px-5 py-3">
                    {r.redeemedAt ? (
                      <span className="text-xs font-semibold text-green-500 bg-green-500/10 border border-green-500/20 px-2.5 py-1 rounded-full uppercase tracking-wide">Redeemed</span>
                    ) : (
                      <span className="text-xs font-semibold text-orange-500 bg-orange-500/10 border border-orange-500/20 px-2.5 py-1 rounded-full uppercase tracking-wide">Pending</span>
                    )}
                  </td>
      ...
      <div className="bg-[#1a1a1a] border border-white/10 rounded-[20px] overflow-hidden">
        <div className="px-5 py-4 border-b border-white/10">
          <h2 className="font-semibold text-white text-sm uppercase tracking-wider">Transaction History</h2>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/5">
              ...
              <tr key={tx.id} className="hover:bg-white/5 transition-colors">
                <td className="px-5 py-3 text-gray-300">
                  ...
                  <span className="text-gray-600">
                    {new Date(tx.effectiveDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </td>
                <td className="px-5 py-3 text-gray-400">{tx.commodityType}</td>
