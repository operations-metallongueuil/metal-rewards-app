...
export default async function CustomersPage({ searchParams }: { searchParams: { search?: string } }) {
  ...
  const customerStats = customers.map((c) => {
    ...
    const lastTx = [...c.transactions].sort((a, b) => b.effectiveDate.getTime() - a.effectiveDate.getTime())[0]
    return { id: c.id, name: c.name, currentPoints, totalLbs, pendingCount, redeemedCount, lastTxDate: lastTx?.effectiveDate ?? null }
  })
    ...
    <div className="p-6">
      ...
      <form className="mb-5">
        <input
          ...
          className="w-full max-w-sm px-4 py-2.5 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-xl text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 text-sm focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors"
        />
      </form>
      ...
      <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-[20px] overflow-hidden shadow-sm dark:shadow-none transition-colors">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-black/20">
            ...
            {customerStats.map((c) => (
              <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                <td className="px-5 py-3.5">
                  <Link href={`/customers/${c.id}`} className="font-medium text-gray-900 dark:text-gray-100 hover:text-orange-500 transition-colors">
                    {c.name}
                  </Link>
                  {c.pendingCount > 0 && (
                    <span className="ml-2 bg-orange-50 dark:bg-orange-500/15 text-orange-600 dark:text-orange-500 border border-orange-200 dark:border-orange-500/20 text-xs px-1.5 py-0.5 rounded-full font-semibold">
                      {c.pendingCount} owed
                    </span>
                  )}
                </td>
                <td className="px-5 py-3.5"><ProgressBar currentPoints={c.currentPoints} /></td>
                <td className="px-5 py-3.5 text-right text-gray-500 dark:text-gray-400">{c.totalLbs.toLocaleString()} lbs</td>
                <td className="px-5 py-3.5 text-right text-gray-700 dark:text-gray-300">{c.pendingCount}</td>
