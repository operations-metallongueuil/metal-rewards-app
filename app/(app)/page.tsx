  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-7">
        <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-[20px] p-5 shadow-sm dark:shadow-none transition-colors">
        ...
        <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-[20px] p-5 shadow-sm dark:shadow-none transition-colors">
          ...
          <p className={`text-3xl font-bold ${pendingRewards > 0 ? 'text-orange-500' : 'text-gray-900 dark:text-white'}`}>
            {pendingRewards}
          </p>
        </div>
        <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-[20px] p-5 shadow-sm dark:shadow-none transition-colors">
          <p className="text-xs text-gray-500 uppercase tracking-wider font-medium mb-2">Near 10 pts (7+)</p>
          <p className="text-3xl font-bold text-orange-500/80">{nearThreshold.length}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-[20px] overflow-hidden shadow-sm dark:shadow-none transition-colors">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/10">
            <h2 className="font-semibold text-gray-900 dark:text-white text-sm uppercase tracking-wider">Rewards to Issue</h2>
            <Link href="/rewards" className="text-xs text-orange-500 hover:text-orange-600 font-medium uppercase tracking-wider transition-colors">
              View all
            </Link>
          </div>
          {withPendingRewards.length === 0 ? (
            <p className="px-5 py-6 text-sm text-gray-400">No pending rewards</p>
          ) : (
            <ul className="divide-y divide-gray-100 dark:divide-white/5">
              {withPendingRewards.map((c) => (
                <li key={c.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  <Link href={`/customers/${c.id}`} className="text-sm font-medium text-gray-700 dark:text-gray-200 hover:text-orange-500 transition-colors">
                    {c.name}
                  </Link>
                  <span className="bg-orange-50 dark:bg-orange-500/15 text-orange-600 dark:text-orange-500 border border-orange-200 dark:border-orange-500/20 text-xs font-semibold px-2.5 py-1 rounded-full">
        ...
        <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-[20px] overflow-hidden shadow-sm dark:shadow-none transition-colors">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/10">
            <h2 className="font-semibold text-gray-900 dark:text-white text-sm uppercase tracking-wider">Near Threshold</h2>
            <Link href="/customers" className="text-xs text-orange-500 hover:text-orange-600 font-medium uppercase tracking-wider transition-colors">
              View all
            </Link>
          </div>
          {nearThreshold.length === 0 ? (
            <p className="px-5 py-6 text-sm text-gray-400">No customers near threshold</p>
          ) : (
            <ul className="divide-y divide-gray-100 dark:divide-white/5">
              {nearThreshold.map((c) => (
                <li key={c.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
